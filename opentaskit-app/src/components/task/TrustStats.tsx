    ...
    <View className="flex-row flex-wrap gap-2.5">
      {stats.map((stat) => (
        ...
        >
          ...
          <Text className="mt-1 text-[17px] font-geist-bold tracking-tight text-ink">
            {stat.value}
          </Text>
        </View>
      ))}
...
export function InlineStat({ label, value }: Stat) {
  return (
    <View className="flex-1 items-center">
      <Text className="text-[16px] font-geist-bold tracking-tight text-ink">{value}</Text>
      <Text className="mt-0.5 text-[11px] font-geist text-ink-400">{label}</Text>
    </View>
  );
}
